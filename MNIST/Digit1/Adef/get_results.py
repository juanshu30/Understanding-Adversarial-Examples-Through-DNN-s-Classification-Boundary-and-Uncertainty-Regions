...
for i in range(1, 11):
    ...
    generate_model['pretrained_model' + str(
        i)] = "/home/shu30/research/paper/model/net_model" + str(i) + ".pkl"
...
original_image1 = np.loadtxt(
    "/home/shu30/research/paper/image/original_image/image1.txt").reshape(28, 28)

# Load the adversarial images
generate_image_var = locals()
for i in np.array((0, 2, 3, 4, 5, 6, 7, 8, 9)):
    image_var = "image1_"+str(i)
    generate_image_var["image1_"+str(i)] = np.unique(np.loadtxt(
        "/home/shu30/research/paper/image/adversarial_image/image1/ADef/image_ADef_tar" + str(i)).reshape(100, 28, 28), axis=0)
...
def IntervalMethods(image_1, adver_image, attackedModel):
    ...
    position_freq100 = position_freq[
        position_freq[:, 1] >= len(adver_image)-10,
    ...
    adver_image_784 = adver_image.reshape(len(adver_image), 784)
    ...
    miss_rate_ori_adver = sum(pred_ori_adver != 1) / len(adver_image)

    # generate new adversarial images based on the entire interval
    p = 0
    pred_neiLR = np.zeros(10)
    for k in np.arange(max(pixels_length - 100,0),
                       pixels_length, 10):
        adver_add_ori = np.zeros(shape=(1000, 28, 28))
        ...
        pred_neiLR[p] = sum(predict_whole != 1) / len(predict_whole)
        p += 1
...
#       generate new adversarial images based on the (min-0.05, max+0.05)
    ...
    for k in np.arange(max(pixels_length - 100,0),
        ...
        for j in np.arange(0, 1000, 1):
            a_image = copy.deepcopy(original_image1).reshape(784)
            for i in np.arange(0, len(main_dim_sorted[0:k]), 1):
                a_image[int(main_dim_sorted[i])] = np.random.uniform(max(interval_table_sorted[:, 0][i] - 0.05, 0),
                                                                     min(interval_table_sorted[:, 1][i] + 0.05, 1))
            adver_add_ori[j] = a_image.reshape(28, 28)
        ...
        pred_neiLR_05[p] = sum(predict_whole != 1) / len(predict_whole)
        p += 1
    ...
    for k in np.arange(max(pixels_length - 100,0),
        ...
        for j in np.arange(0, 1000, 1):
            a_image = copy.deepcopy(original_image1).reshape(784)
            for i in np.arange(0, len(main_dim_sorted[0:k]), 1):
                a_image[int(main_dim_sorted[i])] = np.random.uniform(max(interval_table_sorted[:, 0][i] - 0.1, 0),
                                                                     min(interval_table_sorted[:, 1][i] + 0.1, 1))
            adver_add_ori[j] = a_image.reshape(28, 28)
        ...
        pred_neiLR_01[p] = sum(predict_whole != 1) / len(predict_whole)
        p += 1

    return (pixels_freq100, pixels_length, interval_table_sorted, main_dim_sorted, miss_rate_ori_adver,pred_neiLR, pred_neiLR_05, pred_neiLR_01)



image_list = (image1_0, image1_2, image1_3, image1_4,
...
file=open(r"/./results_ADef.bin","wb")
